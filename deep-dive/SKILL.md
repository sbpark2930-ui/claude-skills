...
argument-hint: "<problem or exploration target> [--lanes N]"
triggers:
  - "deep dive"
  - "deep-dive"
  - "trace and interview"
...
4. **이전 Trace 참조 확인**:
   - `.omc/specs/deep-dive-trace-*.md` 파일을 검색하여 동일 영역의 이전 trace 존재 여부 확인
   - 이전 trace가 발견되면:
     ```
     이전 조사 발견: {date}에 {area}에 대한 trace가 있습니다.
     이전 결론: {most_likely_explanation}
     이번 조사에서 이전 결과를 참고합니다.
     ```
   - 이전 findings를 lane 프롬프트에 컨텍스트로 주입 (중복 조사 방지)
   - state에 저장: `previous_traces: [{path, date, summary}]`

5. **적응형 Lane 수 결정** (기본 3개, `--lanes N`으로 오버라이드 가능):

   | 문제 유형 | Lane 수 | Lane 구성 |
   |-----------|---------|-----------|
   | 단순/명확한 문제 | 2 | Code-path + Config/env |
   | 표준 문제 (기본) | 3 | Code-path + Config/env + Measurement/assumption |
   | 복잡한 시스템 이슈 | 4 | 위 3개 + 데이터 흐름 |
   | 분산/비동기 이슈 | 5 | 위 4개 + 시간/순서 의존 |

   **자동 감지 로직:**
   - 키워드 "간단", "단순", "한 파일" → 2 lanes
   - 키워드 "분산", "비동기", "race condition", "타이밍" → 4-5 lanes
   - 코드베이스 파일 50개 미만 → 최대 3 lanes
   - `--lanes N` 플래그로 수동 오버라이드

6. **Generate trace lane hypotheses**:
   - Default lanes (unless the problem strongly suggests a better partition):
     ...
     4. *(4-lane 이상)* **Data flow / transformation cause**
     5. *(5-lane)* **Timing / ordering dependency cause**
...
Use **Claude built-in team mode** to run N parallel tracer lanes (N = 적응형 lane 수):

1. **Restate the observed result** or "why" question precisely
2. **Spawn N tracer lanes** — one per confirmed hypothesis
3. Each tracer worker must:
   - Own exactly one hypothesis lane
   - Gather evidence **for** the lane
   - Gather evidence **against** the lane
   - **증거 품질 스코어링**: 각 evidence에 quality score 부여:
     - 1.0: 코드에서 직접 확인 (Read 도구로 검증)
     - 0.8: git log/blame에서 확인
     - 0.6: 로그/에러 메시지에서 추론
     - 0.4: 패턴 매칭으로 추론
     - 0.2: 가설 (코드 확인 불가)
   - Name the **critical unknown** for the lane
   - Recommend the best **discriminating probe**
4. **Lane 완료 시 중간 보고** (각 lane 완료 즉시 출력):
   ```
   Lane {N}/{total} 완료: [{가설 제목}]
     핵심 발견: {한 줄 요약}
     신뢰도: {High/Medium/Low}
     증거 품질 평균: {avg_quality:.1f}/1.0
     {강한 증거 발견 시: "유력한 단서를 발견했습니다"}
   ```
5. **Run a rebuttal round** between the leading hypothesis and the strongest alternative
6. **Detect convergence**: if two "different" hypotheses reduce to the same mechanism, merge them explicitly
7. **Leader synthesis**: produce the ranked output below
   - 증거 품질이 높은 lane에 더 높은 가중치
   - 증거 품질 평균 < 0.4인 lane은 `[⚠️ 약한 증거]` 경고 표시
...
## Evidence Summary by Hypothesis (with Quality Scores)
- **Hypothesis 1**: ... | Avg Quality: {score}/1.0
- **Hypothesis 2**: ... | Avg Quality: {score}/1.0
- **Hypothesis 3**: ... | Avg Quality: {score}/1.0
{추가 lane이 있으면 계속}
