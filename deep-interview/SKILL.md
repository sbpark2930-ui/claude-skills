...
### Step 2b-1: 사용자 응답 품질 감지

사용자 응답을 분석하여 품질을 감지한다:

**Low-quality 패턴:**
- "아무거나", "모르겠다", "알아서 해줘", "상관없어", "글쎄", "아직 생각 안 해봤어"
- 1~3 단어 이하의 극단적으로 짧은 응답
- 이전 질문을 그대로 되돌려 묻는 응답 ("그건 뭐가 좋을까요?")

**감지 시 처리:**
1. 해당 라운드를 clarity scoring에서 제외 (점수 변동 없음)
2. `low_quality_responses` 카운터 +1
3. 현재 차원을 임시 보류하고 다른 차원으로 전환
4. 2~3 라운드 후 보류된 차원을 시나리오 기반 질문으로 재시도

**3회 연속 low-quality 시:**
```
구체적인 예시로 질문 방식을 바꿔볼까요?
예: "사용자 A가 이렇게 행동하면 어떻게 되어야 하나요?" 같은 시나리오 기반 질문으로 전환합니다.
```

### Step 2b-2: 시나리오 기반 질문 모드

다음 조건에서 시나리오 모드가 활성화된다:
- `low_quality_responses` >= 2
- Goal clarity < 0.5
- 사용자가 "예시로 물어봐" 라고 요청

**시나리오 질문 형식:**
추상적 질문 대신 구체적 시나리오를 제시한다:
```
일반 질문: "검색 기능이 어떻게 동작해야 하나요?"
시나리오 질문: "사용자 A가 검색창에 '파이썬 튜토리얼'을 입력하고 Enter를 누릅니다.
  1. 어떤 결과가 먼저 보여야 하나요?
  2. 결과가 없으면 어떤 화면이 나와야 하나요?
  3. 오타('파이선')를 입력하면 자동 수정을 해야 하나요?"
```

시나리오 모드에서 얻은 답변으로 추상적 차원의 clarity를 간접적으로 향상시킨다.
...
**Ontology Mermaid ER 다이어그램** (entity 3개 이상일 때):
```mermaid
erDiagram
    User ||--o{ Order : "places"
...
```
매 라운드마다 entity가 변경되면 ER 다이어그램을 업데이트하여 표시한다.
최종 spec 문서에도 포함한다.
...
### Step 2e: Update State + Spec 버전 스냅샷

Update interview state with the new round and scores via `state_write`.

**자동 스냅샷 조건:**
- Round 5, 10, 15 도달 시 자동 스냅샷
- Goal clarity가 한 라운드에서 0.2 이상 하락 시 (방향 전환 감지)
- 사용자가 "스냅샷 저장" 라고 요청 시
...
```json
{
  "spec_snapshots": [
    {
      "round": 5,
      "trigger": "auto_round_5",
      "ambiguity": 0.45,
      "goal_summary": "작업 관리 앱 — CRUD + 태그",
      "key_entities": ["User", "Task", "Tag"],
      "constraints": ["오프라인 지원", "모바일 우선"]
    }
  ]
}
```

사용자가 "이전 버전과 비교해줘" 라고 하면 현재 spec과 가장 최근 스냅샷을 diff로 표시:
```
Spec 변경 비교 (Round 5 → Round 8):
  Goal: "작업 관리 앱" → "작업 관리 + 협업 플랫폼" (확장됨)
  새 entity: +Workspace, +Team
  삭제된 constraint: -"오프라인 지원" (Round 7에서 제거)
  새 constraint: +"실시간 동기화"
```
...
### Round 10+: Persona Mode (if ambiguity still > 0.25)
3개 페르소나 관점에서 현재 spec을 검증한다:

- **최종 사용자 관점**: "비기술 사용자가 이 기능을 처음 접하면 직관적인가? 에러 시 사용자가 이해할 수 있는 메시지를 받는가?"
- **운영팀 관점**: "프로덕션에서 모니터링이 가능한가? 장애 시 디버깅할 수 있는 정보가 충분한가? 알림 기준은?"
- **보안팀 관점**: "인증/인가 갭이 있는가? 데이터 노출 위험이 있는가? 규정 준수 요건은?"

각 페르소나에서 발견된 gap을 추가 질문으로 제시한다.
Persona Mode는 1회만 사용하며, 3개 관점을 순차적으로 질문한다.
