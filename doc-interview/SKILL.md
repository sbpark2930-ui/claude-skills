...
3. **사용자에게 프로젝트 설계 파이프라인을 제안한다:**

```
현재 디렉토리에 소스 코드가 없습니다. 새 프로젝트를 시작하시는 것 같습니다.

코드 없이도 프로젝트 설계 문서를 작성할 수 있습니다.
어떻게 진행하시겠습니까?

  1. 프로젝트 설계 파이프라인 (Recommended)
     → PRD → 기술 설계서 → ADR 순서로 프로젝트 전체를 설계합니다.
     → 완료 후 deep-interview 또는 plan-interview로 구현 계획을 세울 수 있습니다.

  2. 단일 문서만 작성
     → PRD, 기술 설계서, ADR 중 하나만 선택하여 작성합니다.

  3. 요구사항 정제부터 (deep-interview)
     → 아이디어가 아직 모호하다면 deep-interview로 요구사항을 먼저 정제합니다.
     → 정제 완료 후 doc-interview로 돌아와 문서를 작성합니다.
```

**선택 1: 프로젝트 설계 파이프라인 모드**

greenfield 프로젝트에 가장 적합한 흐름이다. 3단계 순차 문서 작성 파이프라인:

```
Phase A: PRD (요구사항 정의)
  "무엇을 만들 것인가?" — 문제, 사용자, 기능, 수용 기준
     ↓ (PRD 완성 → 다음 문서의 컨텍스트로 자동 주입)
Phase B: 기술 설계서 (아키텍처 결정)
  "어떻게 만들 것인가?" — 아키텍처, 데이터 모델, API 계약
     ↓ (설계서 완성 → 다음 문서의 컨텍스트로 자동 주입)
Phase C: ADR (핵심 의사결정 기록)
  "왜 이렇게 결정했는가?" — 선택지, 근거, 예상 결과
```

- 각 Phase에서 이전 Phase의 문서를 `codebase_context`에 자동 주입한다.
  - Phase B 시작 시: PRD의 기능 목록, 수용 기준을 자동 참조
  - Phase C 시작 시: PRD + 기술 설계서의 기술 결정을 자동 참조
- Phase 사이에 `AskUserQuestion`으로 계속 진행 여부를 확인한다:
  ```
  PRD 작성이 완료되었습니다. (.omc/docs/prd-{slug}.md)

  다음 단계:
    1. 기술 설계서 작성으로 진행
    2. 여기서 멈추고 나중에 계속
    3. deep-interview로 요구사항 더 정제
    4. plan-interview로 구현 계획 수립
  ```
- 파이프라인 상태를 state에 저장:
  ```json
  {
    "pipeline_mode": true,
    "pipeline_phases": ["prd", "tech-design", "adr"],
    "pipeline_current": 0,
    "pipeline_outputs": {
      "prd": ".omc/docs/prd-{slug}.md",
      "tech-design": null,
      "adr": null
    }
  }
  ```
- 모든 Phase 완료 후 최종 핸드오프:
  ```
  프로젝트 설계 파이프라인이 완료되었습니다.

  생성된 문서:
    ✓ PRD: .omc/docs/prd-{slug}.md
    ✓ 기술 설계서: .omc/docs/tech-design-{slug}.md
    ✓ ADR: .omc/docs/adr-{slug}.md

  다음 단계:
    1. deep-interview → plan-interview → autopilot (전체 구현 파이프라인)
    2. plan-interview → ralph (설계 납득 → 구현)
    3. 완료 (문서만 보관)
  ```
...
프로젝트 설계 파이프라인(PRD → 기술 설계서 → ADR)으로 전환하시겠습니까?
```

**선택 3: deep-interview로 전환**

아이디어가 모호한 경우 `Skill("oh-my-claudecode:deep-interview")`로 핸드오프한다.
deep-interview 완료 후 생성된 spec을 `codebase_context`에 주입하여 doc-interview를 재시작할 수 있다.

4. **Greenfield 인터뷰 전략** (선택 1, 2 공통):
   - 코드에서 추출할 사실이 없으므로 **초기 앵커 질문**으로 기본 컨텍스트를 확보한다:
     ```
     프로젝트를 이해하기 위해 기본 질문부터 시작합니다:
